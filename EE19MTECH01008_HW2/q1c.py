"""
Q1)(c) Implement MLP OR gate
Author- Subhra Shankha Bhattacherjee
Roll - EE19MTECH01008
"""
"""
Note to the evaluator: please re-run the code if it does not predict correctly in first execution
"""
import numpy as np 
# import random
...
def layer(w,x,b):
	a = np.dot(x,w)+b
	return a
print("This MLP OR gate network has 2 input nodes, 1 hidden layer with 2 nodes and 1 output node. Noise is generated from 0 mean 0.5 standard deviation gaussian distribution. 100 samples for each bit is generated. The network has a learning rate of 0.1 and is run for 800 epochs \n")
...
noise_std = 0.5  #standard deviation for noise generation

#XOR dataset
true_inpt = np.array([[0,0],[0,1],[1,0],[1,1]])
true_opt = np.array([[0],[1],[1],[1]])
...
for _ in range(0,4):
    ...
    if(y_pred >= 0.2): # Again I got this range based on prediction values.
        y_pred = 1
    else :
        y_pred = 0
