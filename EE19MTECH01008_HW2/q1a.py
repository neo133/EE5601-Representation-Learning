"""
Q1)(a) Implement MLP XOR gate
Author- Subhra Shankha Bhattacherjee
Roll - EE19MTECH01008
"""
"""
Note to the evaluator: sometimes the network doesn't predict correctly. Please re-run the code once again in that case.
"""
import numpy as np 
import random
...
def layer(w,x,b):
	a = np.dot(x,w)+b
	return a
print("This MLP XOR gate network has 2 input nodes, 1 hidden layer with 2 nodes and 1 output node. Noise is generated from 0 mean, 0.05 standard deviation gaussian distribution. 100 samples for each bit is generated. The network has a learning rate of 0.01 and is run for 800 epochs \n")
...
noise_std = 0.05  #standard deviation for noise generation

#XOR dataset
true_inpt = np.array([[0,0],[0,1],[1,0],[1,1]])
true_opt = np.array([[0],[1],[1],[0]])
...
#input using comma separated like 0,0 or 1,1 
for _ in range(0,4):
    a=raw_input("test case (enter any one of 4 input combinations (0,0), (0,1), (1,0), (1,1) comma separated): ").split(',')
    for i in range(0,2):
        a[i]=float(a[i])
    a=np.array(a)
    out_1 = layer(hid_w,a,hid_b)
    z = sigmoid(out_1)
    out_2 = layer(opt_w,z,opt_b)
    y_pred= sigmoid(out_2)
    if(y_pred >= 0.50 and y_pred <= 0.51): # I was having some issues, i got this range based on prediction values.
        y_pred = 1
    else :
        y_pred = 0
